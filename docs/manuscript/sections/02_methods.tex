...
Starting from \FlowAllED{} ED stays in the extract, we applied diagnosis-based inclusion, excluded small or unknown race/ethnicity categories, required a positive initial pain score with valid time-to-event information, and required a documented Emergency Severity Index (ESI) triage level.
Insurance categories in models included private (reference), Medicare, Medicaid, undocumented (missing payer documentation in the EHR), and uninsured (self-pay).
